...
public class DrawingPanel extends JPanel
{
	...
	// Deklaration der Transformationsmatrizen (invertierte Reihenfolge der Berechnung)
	...
	protected void paintComponent(Graphics g)
	{
		...
		MainMatrixTransformation.translation_view.setMatrix(translation_);
		MainMatrixTransformation.back_view.setMatrix(cent_back_translation_);
		MainMatrixTransformation.rotation_view.setMatrix(rot_mat);
		MainMatrixTransformation.scale_view.setMatrix(scaling_);
		MainMatrixTransformation.center_view.setMatrix(cent_translation_);
		MainMatrixTransformation.total_view.setMatrix(t_ges);
		
		MainMatrixTransformation.alpha_view.setMatrix(x_rotation_);
		MainMatrixTransformation.beta_view.setMatrix(y_rotation_);
		MainMatrixTransformation.gamma_view.setMatrix(z_rotation_);
		
	}
	
	public void xCenterTranslation(float slider_curr_value) {
		cent_translation_[0][3] = slider_curr_value * (-1);
		cent_back_translation_[0][3] = cent_translation_[0][3] * (-1);
		
		repaint();
	}
	public void yCenterTranslation(float slider_curr_value) {
		cent_translation_[1][3] = slider_curr_value * (-1);
		cent_back_translation_[1][3] = cent_translation_[1][3] * (-1);
		
		repaint();
	}
	public void zCenterTranslation(float slider_curr_value) {
		cent_translation_[2][3] = slider_curr_value * (-1);
